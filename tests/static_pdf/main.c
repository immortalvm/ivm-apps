...
static int stdout_fn(void *caller_handle, const char *str, int len)
{
    //static FILE* f = NULL;
    //if (f == NULL) f = fopen("g.out", "w");
    //fprintf(f, "%s", str);
    lprintf1("%s", str);
    return len;
}

static int stderr_fn(void *caller_handle, const char *str, int len)
{
    lprintf1("%s", str);
    printf("ERROR: %s\n",str);
    return len;
...
int main(int argc, char *argv[])
{
    ...
    gsargv[ac++] = "-dGraphicsAlphaBits=4";
    gsargv[ac++] = "-dTextAlphaBits=4";
    gsargv[ac++] = "-r300x300";
#if IVM64
