...
pushd boxing
CC=ivm64-gcc  CFLAGS="-DBOXING_USE_C99_LIBRARIES" ./configure --host ivm64 --prefix=$IVMROOT/ivm-apps/boxing
make && make install
popd

git clone https://github.com/preservationvm/afs.git
pushd afs
CC=ivm64-gcc CFLAGS="-DBOXING_USE_C99_LIBRARIES" ./configure --host ivm64 --prefix=$IVMROOT/ivm-apps/boxing  LIBBOXING_DIR=$IVMROOT/ivm-apps/boxing
make && make install
popd
...
pushd zlib-1.2.11 
CC=ivm64-gcc ./configure --static --prefix=$IVMROOT/ivm-apps/file-format-decoders
make && make install
popd

git clone https://github.com/preservationvm/tiff-v4.1.0
pushd tiff-v4.1.0
CC=ivm64-gcc ./configure --host ivm64 --prefix=$IVMROOT/ivm-apps/file-format-decoders
make && make install
popd
